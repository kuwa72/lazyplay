// FairPlay SAP wrapper. Adapted from UxPlay lib/fairplay_playfair.c (LGPLv2.1+),
// with the logger dependency removed.
#include "fairplay.h"

#include <cstring>

extern "C" {
#include "playfair/playfair.h"
}

namespace {

const uint8_t REPLY_MESSAGE[4][142] = {
    {},
    {},
    {},
    {}
};

const uint8_t FP_HEADER[12] = { 0x46, 0x50, 0x4c, 0x59, 0x03, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x14 };

} // namespace

int FairPlay::Setup(const uint8_t req[16], uint8_t res[142]) {
    if (req[4] != 0x03) {
        return -1; // Unsupported FairPlay version
    }
    int mode = req[14];
    if (mode < 0 || mode > 3) {
        return -1;
    }
    memcpy(res, REPLY_MESSAGE[mode], 142);
    m_keymsgLen = 0;
    return 0;
}

int FairPlay::Handshake(const uint8_t req[164], uint8_t res[32]) {
    if (req[4] != 0x03) {
        return -1;
    }
    memcpy(m_keymsg, req, 164);
    m_keymsgLen = 164;
    memcpy(res, FP_HEADER, 12);
    memcpy(res + 12, req + 144, 20);
    return 0;
}

int FairPlay::Decrypt(const uint8_t input[72], uint8_t output[16]) {
    if (m_keymsgLen != 164) {
        return -1;
    }
    playfair_decrypt(const_cast<uint8_t*>(m_keymsg), const_cast<uint8_t*>(input), output);
    return 0;
}
